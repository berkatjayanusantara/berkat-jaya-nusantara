    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                @if ($penjualan->piutang->status_piutang === 'lunas')
                <div class="mb-4 p-4 bg-green-100 text-green-800 rounded-md">
                    Penjualan ini sudah <strong>LUNAS</strong> melalui riwayat pembayaran. Metode pembayaran tidak dapat diubah menjadi tunai untuk menjaga konsistensi data riwayat pembayaran.
                </div>
                @else
                ...
                <form action="{{ route('invoice-historis.penjualan.update', $penjualan->id_penjualan) }}" method="POST" id="formPenjualan">
                    @csrf
                    @method('PUT')
                                ...
                                <tbody>
                                    ...
                                    <tr data-old-barang-id="{{ $detail->id_barang }}" data-old-jumlah="{{ $detail->jumlah }}">
                                        ...
                                        <td class="border px-3 py-2 min-w-[150px]">
                                            <input type="number" name="diskon_nominal[]" value="{{ old('diskon_nominal.' . $loop->index, $detail->diskon_nominal ?? 0) }}" min="0" step="0.01" class="w-full border-gray-300 rounded-md shadow-sm text-right diskon-input" placeholder="0">
                                            <p class="text-xs text-green-600 mt-1 diskon-info text-right">-</p>
                                        </td>
                                        <td class="border px-3 py-2 min-w-[150px]">
                                            <input type="date" name="tanggal_pengantaran[]" value="{{ old('tanggal_pengantaran.' . $loop->index, optional($detail->tanggal_pengantaran)->format('Y-m-d')) }}" class="w-full border-gray-300 rounded-md shadow-sm text-center">
                                        </td>
                    ...
                    <div class="flex justify-end gap-2 mt-6">
                        <a href="{{ route('invoice-historis.penjualan.show', ['penjualan' => $penjualan->id_penjualan, 'back_url' => route('invoice-historis.index')]) }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Batal</a>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700" onclick="return confirm('Update invoice penjualan lama ini? Data ini tetap tidak akan mengurangi stok barang saat ini.')">Update Invoice Lama</button>
                    </div>
                </form>
            </div>
        </div>
