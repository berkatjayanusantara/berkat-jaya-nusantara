<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('invoice-historis.penjualan.store') }}" method="POST" id="formPenjualan">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_penjualan" value="{{ old('tanggal_penjualan', date('Y-m-d')) }}" class="w-full border-gray-300 rounded-md shadow-sm" required>
                        </div>
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Pilih customer lama atau tambah customer baru jika belum terdaftar.</p>
                        </div>
                    </div>
                        ...
                        <div>
                            <div class="mb-4">
                                ...
                                <select name="metode_pembayaran" id="metodePembayaran" class="w-full border-gray-300 rounded-md shadow-sm" required>
                                    ...
                                    <option value="kredit" {{ old('metode_pembayaran') === 'kredit' ? 'selected' : '' }}>Kredit / Piutang</option>
                                </select>
                            </div>
                            <div class="mb-4" id="fieldJatuhTempo" style="display: none;">
                                <label class="block mb-1 font-medium">Tanggal Jatuh Tempo</label>
                                <input type="date" name="tanggal_jatuh_tempo" value="{{ old('tanggal_jatuh_tempo') }}" class="w-full border-gray-300 rounded-md shadow-sm">
                                <p class="text-sm text-gray-500 mt-1">Wajib diisi jika pembayaran kredit.</p>
                            </div>
                            <div>
                                <label class="block mb-1 font-medium">Catatan</label>
                                <textarea name="catatan" rows="4" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('catatan') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-md border">
                            <div class="mb-4">
                                ...
                                <select name="mode_ppn"
                                    ...
                                    <option value="tanpa_ppn" id="optionTanpaPpn" {{ old('mode_ppn') === 'tanpa_ppn' ? 'selected' : '' }}>Tidak Pakai PPN</option>
                                    <option value="include" {{ old('mode_ppn', 'include') === 'include' ? 'selected' : '' }}>Harga Sudah Termasuk PPN</option>
                                    <option value="exclude" {{ old('mode_ppn') === 'exclude' ? 'selected' : '' }}>Harga Belum Termasuk PPN</option>
                            ...
                            <div class="mb-4 p-3 bg-white rounded border">
                                ...
                                <label class="flex items-start gap-2">
                                    <input type="checkbox"
                                        ...
                                        value="1"
                                        class="mt-1"
                                        {{ old('butuh_faktur_pajak') ? 'checked' : '' }}>
                                    <span>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <select name="jenis_penyesuaian_total" id="jenisPenyesuaianTotal" class="w-full border-gray-300 rounded-md shadow-sm">
                                            ...
                                            <option value="tidak_ada" {{ old('jenis_penyesuaian_total', 'tidak_ada') === 'tidak_ada' ? 'selected' : '' }}>Tidak Ada</option>
                                            <option value="tambah" {{ old('jenis_penyesuaian_total') === 'tambah' ? 'selected' : '' }}>Tambah Total Akhir</option>
                                            <option value="kurang" {{ old('jenis_penyesuaian_total') === 'kurang' ? 'selected' : '' }}>Kurangi Total Akhir</option>

                                        </select>
                                    </div>
                                    <div>
                                        <label class="block mb-1 text-sm font-medium">Nominal Penyesuaian</label>
                                        <input type="number" name="nominal_penyesuaian_total" id="nominalPenyesuaianTotal" value="{{ old('nominal_penyesuaian_total', 0) }}" min="0" step="0.01" class="w-full border-gray-300 rounded-md shadow-sm text-right">
                                    </div>
                                </div>
                                <div class="mt-3">
                                    <label class="block mb-1 text-sm font-medium">Keterangan Penyesuaian</label>
                                    <textarea name="keterangan_penyesuaian_total" id="keteranganPenyesuaianTotal" rows="2" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('keterangan_penyesuaian_total') }}</textarea>
                                </div>
                            </div>
                    ...
                    <div class="flex justify-end gap-2 mt-6">
                        <a href="{{ route('invoice-historis.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Batal</a>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700" onclick="return confirm('Simpan invoice penjualan lama ini? Data ini tidak akan mengurangi stok barang saat ini.')">Simpan Invoice Lama</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('btn-hapus')) {
                ...
                if (tbody.querySelectorAll('tr').length <= 1) {
                    alert('Minimal harus ada satu barang dalam invoice penjualan lama.');
                    return;
                }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            updateFieldPenyesuaianTotal();
            document.querySelectorAll('#tableBarang tbody tr').forEach(row => updateBarangInfo(row));
            hitungTotal();
